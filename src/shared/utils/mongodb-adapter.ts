import { DatabaseAdapter, DatabaseConfig, Transaction } from '../types/database-adapter';
...
export class MongoDBAdapter implements DatabaseAdapter {
  ...
  async connect(): Promise<void> {
    try {
      ...
      this.client = new MongoClient(this.config.connectionString || `mongodb://${this.config.host}:${this.config.port}/${this.config.database}`, this.config.options);
      ...
      await this.collection.createIndex({ mobile: 1 }, { sparse: true });
  ...
  async findUserByUsername(username: string): Promise<User | null> {
    try {
      const user = await this.collection.findOne({ username: username.toLowerCase() });
      return user ? this.mapToUser(user) : null;
    } catch (error) {
  ...
  async findUsers(query: UserQuery, filters?: UserFilters, sort?: UserSort, pagination?: UserPagination): Promise<UserListResult> {
    try {
      const filter: any = {};
      ...
      if (query.isEmailVerified !== undefined) filter.isEmailVerified = query.isEmailVerified;
      if (query.isMobileVerified !== undefined) filter.isMobileVerified = query.isMobileVerified;
      ...
      if (filters?.isEmailVerified !== undefined) filter.isEmailVerified = filters.isEmailVerified;
      if (filters?.isMobileVerified !== undefined) filter.isMobileVerified = filters.isMobileVerified;
      if (filters?.createdAfter) filter.createdAt = { $gte: filters.createdAfter };
      if (filters?.createdBefore) filter.createdAt = { ...filter.createdAt, $lte: filters.createdBefore };
      if (filters?.lastLoginAfter) filter.lastLoginAt = { $gte: filters.lastLoginAfter };
      if (filters?.lastLoginBefore) filter.lastLoginAt = { ...filter.lastLoginAt, $lte: filters.lastLoginBefore };
      ...
      const [users, total] = await Promise.all([
        this.collection.find(filter).sort(sortObj).skip(skip).limit(limit).toArray(),
  ...
  async createUser(userData: CreateUserData): Promise<User> {
    try {
      ...
      const result = await this.collection.insertOne(this.mapToMongo(user));
      return user;
    } catch (error) {
  ...
  async updateLastLogin(id: string, lastLoginAt: Date): Promise<void> {
    try {
      await this.collection.updateOne(
        { _id: id },
        { $set: { lastLoginAt, loginAttempts: 0, updatedAt: new Date() } }
      );
    } catch (error) {
  ...
  async createUsers(usersData: CreateUserData[]): Promise<User[]> {
    try {
      ...
      const result = await this.collection.insertMany(users.map(this.mapToMongo));
      return users;
    } catch (error) {
  ...
  async updateUsers(ids: string[], userData: UpdateUserData): Promise<User[]> {
    try {
      const updateData: any = {
        ...userData,
        updatedAt: new Date()
  ...
  async countUsers(query?: UserQuery, filters?: UserFilters): Promise<number> {
    try {
      const filter: any = {};
  ...
  async userExists(email?: string, username?: string, mobile?: string): Promise<boolean> {
    try {
      const filter: any = {};
