...
export class ServerManager {
    ...
    private setupSwagger(): void {
        const swaggerOptions = {
                ...
                components: {
                    ...
                    parameters: {
                        RequestIdentifier: {
                            ...
                            in: 'header',
                            description:
                                'Request identifier for tracking (optional - will be auto-generated if not provided)',
                            required: false,
                            schema: {
                                type: 'string',
                        ...
                        CorrelationId: {
                            ...
                            in: 'header',
                            description:
                                'Correlation ID for request tracking (alternative to x-request-id)',
                            required: false,
                            schema: {
                                type: 'string',
                        ...
                        Result: {
                            ...
                            properties: {
                                ...
                                additionalInfo: {
                                    type: 'object',
                                    description:
                                        'Additional information for debugging/auditing',
                                },
                                user: {
                                    type: 'object',
                                    description:
            ...
            apis: [`${process.cwd()}/src/modules/**/*.ts`],
        }
                ...
                Object.keys(spec.paths[path]).forEach((method) => {
                    if (method !== 'parameters') {
                        ...
                        globalParams.forEach((globalParam) => {
                            const paramExists = operation.parameters.some(
                                (param: any) => param.$ref === globalParam.$ref
                            )
    ...
    private setupSocketEvents(): void {
        ...
        this.io.on('connection', (socket) => {
            ...
            for (const [eventName, eventHandler] of this.socketEvents) {
                socket.on(eventName, (data) => {
                    try {
                        ...
                        logger.error(
                            `[Socket.IO] Error handling event ${eventName}:`,
                            error
                        )
                        socket.emit('error', {
            ...
            socket.on('error', (error) => {
                logger.error(
                    `[Socket.IO] Socket error for ${socket.id}:`,
                    error
                )
            })
        })
    ...
    registerSocketEvent(event: SocketEvent): void {
        ...
        logger.info(
            `[Socket.IO] Registered event: ${event.event} - ${event.description || 'No description'}`
        )
    }
    ...
    private registerPluginRoutes(): void {
        ...
        if (this.config.socketio?.enabled) {
            this.app.get('/api/socket-info', (req: Request, res: Response) => {
                const events = Array.from(this.socketEvents.values()).map(
                    (event) => ({
                        event: event.event,
    ...
    private initializeAuthMiddleware(): void {
        try {
            ...
            if (authPlugin) {
                ...
                const authModule =
                    (authPlugin as any).getAuthModule?.() || authPlugin
                AuthMiddleware.initialize(authModule)
                logger.info(
                    '[ServerManager] AuthMiddleware initialized successfully'
                )
            } else {
                logger.warn(
                    '[ServerManager] Auth plugin not found, AuthMiddleware not initialized'
                )
            }
        } catch (error) {
            logger.error(
                '[ServerManager] Failed to initialize AuthMiddleware:',
                error
            )
        }
    }
        ...
        this.app.use(
            (error: any, req: Request, res: Response, next: NextFunction) => {
                ...
                result.addMessage(
                    ...
                    'ServerManager',
                    'setupErrorHandling',
                    'Dangerous input may cause a crash'
                )
                handleResponse(res, result)
            }
        )
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                serverToUse.listen(port, host, () => {
                    logger.info(
                        `[ServerManager] Server started on http://${host}:${port}/api-docs`
                    )
                    if (this.config.socketio?.enabled) {
                        logger.info(
                            `[ServerManager] Socket.IO available at ws://${host}:${port}`
                        )
                    }
                    resolve()
                })
        ...
        return new Promise((resolve, reject) => {
            ...
            if (serverToUse) {
                serverToUse.close((error: any) => {
                    if (error) {
                        logger.error(
                            '[ServerManager] Error stopping server:',
                            error
                        )
                        reject(error)
                    } else {
