...
export class MongoDBConnection implements DatabaseConnection {
  ...
  private setupLoggingConnection(): void {
    ...
    try {
      ...
      console.log(
        "[DEBUG] Available transports:",
        logger.getTransports().map((t) => `${t.name}:${t.isEnabled()}`),
      );
      ...
      console.error(
        "[ERROR] Failed to configure logging database connection:",
        error,
      );
      logger.warn(
        "[MongoDB] Failed to configure logging database connection:",
        error,
      );
    }
  }
    ...
    try {
      ...
      for (const modelName of models) {
        try {
          ...
          if (model.schema.indexes) {
            for (const index of model.schema.indexes()) {
              try {
                ...
                logger.debug(
                  `[MongoDB] Created index for ${collectionName}: ${JSON.stringify(index[0])}`,
                );
              } catch (indexError: any) {
                // Index might already exist, which is fine
                if (indexError.code !== 85) {
                  // 85 is "Index already exists" error
                  logger.warn(
          ...
          logger.warn(
            `[MongoDB] Error processing model ${modelName}:`,
            modelError,
          );
        }
      }
...
export class DatabaseManager {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      logger.info(
        "[DatabaseManager] Database connections initialized successfully",
      );
    } catch (error) {
      logger.error(
        "[DatabaseManager] Failed to initialize database connections:",
        error,
      );
      throw error;
    }
  ...
  async createAllCollections(): Promise<void> {
    for (const [name, connection] of this.connections) {
      try {
        ...
        logger.info(
          `[DatabaseManager] Created collections for connection: ${name}`,
        );
      } catch (error) {
        logger.error(
          `[DatabaseManager] Error creating collections for ${name}:`,
          error,
        );
      }
    }
  ...
  async closeAll(): Promise<void> {
    ...
    for (const [name, connection] of this.connections) {
      try {
        ...
        logger.error(
          `[DatabaseManager] Error closing connection ${name}:`,
          error,
        );
      }
    }
