...
export class DatabaseSeeder {
  ...
  async seed(options: SeederOptions = {}): Promise<void> {
    try {
      ...
      if (!options.force && await this.hasData()) {
        logger.info('[DatabaseSeeder] Database already has data, skipping seeding');
        return;
      }
      ...
      logger.info('[DatabaseSeeder] Database seeding completed successfully');
    } catch (error) {
  ...
  private async seedDynamicModules(seedData: SeedData): Promise<void> {
    ...
    const dynamicKeys = Object.keys(seedData).filter(key => !coreKeys.includes(key));
    ...
    for (const key of dynamicKeys) {
      ...
      if (data && Array.isArray(data) && data.length > 0) {
        ...
        if (typeof (this as any)[methodName] === 'function') {
          ...
          logger.warn(`[DatabaseSeeder] No seeder method found for ${key}, skipping`);
        }
      }
    }
  }
  ...
  private async hasData(): Promise<boolean> {
    try {
      ...
      logger.error('[DatabaseSeeder] Error checking if database has data:', error);
      return false;
    }
  }
    ...
    const defaultData: SeedData = {
      ...
      roles: [
        ...
        { 
          ...
          description: 'Identity manager with permissions management access',
          isActive: true,
          isSystem: false,
          createdBy: 'system',
          updatedBy: 'system'
      ...
      permissions: [
        ...
        { 
          ...
          description: 'Admin can manage modules and basic system functions',
          module: 'SYSTEM',
          action: 'MANAGE',
          isActive: true,
          requiresAuth: true,
        ...
        { 
          ...
          description: 'Full access to permissions manager for super admins',
   ...
   * Seed users
   */
  private async seedUsers(users: any[], options: SeederOptions = {}): Promise<void> {
    ...
    for (const userData of users) {
      try {
        ...
        if (!existing) {
          ...
          await user.save();
          logger.info(`[DatabaseSeeder] Created user: ${userData.email}`);
        } else {
          logger.info(`[DatabaseSeeder] User ${userData.email} already exists, skipping`);
        }
      } catch (error) {
        logger.error(`[DatabaseSeeder] Failed to create user ${userData.email}:`, error);
      }
    }
  }
    ...
    for (const role of roles) {
      try {
        ...
        if (!existing) {
          ...
          await newRole.save();
          logger.info(`[DatabaseSeeder] Created role: ${role.name} (${role.code})`);
        } else {
          logger.info(`[DatabaseSeeder] Role ${role.name} (${role.code}) already exists, skipping`);
        }
      } catch (error) {
        logger.error(`[DatabaseSeeder] Failed to create role ${role.name}:`, error);
      }
    }
  }
  ...
  private async seedPermissions(permissions: any[]): Promise<void> {
    ...
    logger.info(`[DatabaseSeeder] Seeding ${permissions.length} permissions...`);
    ...
    for (const permission of permissions) {
      try {
        ...
        if (!existing) {
          ...
          logger.info(`[DatabaseSeeder] Created permission: ${permission.name} (${permission.code})`);
        } else {
          logger.info(`[DatabaseSeeder] Permission ${permission.name} (${permission.code}) already exists, skipping`);
        }
      } catch (error) {
        logger.error(`[DatabaseSeeder] Failed to create permission ${permission.name}:`, error);
      }
    }
  }
  ...
  private async linkRolesAndPermissions(): Promise<void> {
    try {
      logger.info('[DatabaseSeeder] Linking roles, permissions, and users...');
      
      // Get the super admin role and permission
      const superAdminRole = await RoleModel.findOne({ code: 'SUPER_ADMIN' });
      const superAdminPermission = await PermissionModel.findOne({ code: 'SUPER_ADMIN_ALL' });
      const superAdminUser = await UserModel.findOne({ email: 'admin@domain.com' });
      
      if (superAdminRole && superAdminPermission && superAdminUser) {
        // Link super admin permission to super admin role
        await PermissionModel.findByIdAndUpdate(superAdminPermission._id, {
        ...
        logger.info('[DatabaseSeeder] Successfully linked super admin role, permission, and user');
        logger.info(`[DatabaseSeeder] User ${superAdminUser.email} now has roleIds: [${superAdminUser.roleIds}]`);
      } else {
        logger.warn('[DatabaseSeeder] Could not find all required entities for linking');
        if (!superAdminRole) logger.warn('[DatabaseSeeder] SUPER_ADMIN role not found');
        if (!superAdminPermission) logger.warn('[DatabaseSeeder] SUPER_ADMIN_ALL permission not found');
        if (!superAdminUser) logger.warn('[DatabaseSeeder] admin@domain.com user not found');
      }
      
      // Link other roles and permissions as needed
      const adminRole = await RoleModel.findOne({ code: 'ADMIN' });
      const adminPermission = await PermissionModel.findOne({ code: 'ADMIN_MODULE_MANAGEMENT' });
      ...
      if (adminRole && adminPermission) {
        ...
        logger.info('[DatabaseSeeder] Successfully linked admin role and permission');
      }
      
      const userRole = await RoleModel.findOne({ code: 'USER' });
      const userPermission = await PermissionModel.findOne({ code: 'USER_BASIC' });
      
      if (userRole && userPermission) {
        await PermissionModel.findByIdAndUpdate(userPermission._id, {
          $addToSet: { allowedRoleIds: userRole._id }
        });
        logger.info('[DatabaseSeeder] Successfully linked user role and permission');
      }
      
      // Link permissions manager permission to super admin and identity manager roles
      const permissionsManagerPermission = await PermissionModel.findOne({ code: 'PERMISSIONS_MANAGER_ALL' });
      const identityManagerRole = await RoleModel.findOne({ code: 'IDENTITY_MANAGER' });
      
      if (superAdminRole && permissionsManagerPermission) {
        await PermissionModel.findByIdAndUpdate(permissionsManagerPermission._id, {
          $addToSet: { allowedRoleIds: superAdminRole._id }
        });
        logger.info('[DatabaseSeeder] Successfully linked permissions manager permission to super admin role');
      }
      ...
      if (identityManagerRole && permissionsManagerPermission) {
        ...
        logger.info('[DatabaseSeeder] Successfully linked permissions manager permission to identity manager role');
      }
      
      // Link admin panel access permission to super admin, identity manager, and admin roles
      const adminPanelAccessPermission = await PermissionModel.findOne({ code: 'ADMIN_PANEL_ACCESS' });
      if (superAdminRole && adminPanelAccessPermission) {
        await PermissionModel.findByIdAndUpdate(adminPanelAccessPermission._id, {
          $addToSet: { allowedRoleIds: superAdminRole._id }
        });
        logger.info('[DatabaseSeeder] Successfully linked admin panel access permission to super admin role');
      }
      if (identityManagerRole && adminPanelAccessPermission) {
        await PermissionModel.findByIdAndUpdate(adminPanelAccessPermission._id, {
          $addToSet: { allowedRoleIds: identityManagerRole._id }
        });
        logger.info('[DatabaseSeeder] Successfully linked admin panel access permission to identity manager role');
      }
      if (adminRole && adminPanelAccessPermission) {
        await PermissionModel.findByIdAndUpdate(adminPanelAccessPermission._id, {
          $addToSet: { allowedRoleIds: adminRole._id }
        });
        logger.info('[DatabaseSeeder] Successfully linked admin panel access permission to admin role');
      }

      // Link admin panel full access permission to super admin and identity manager roles
      const adminPanelFullAccessPermission = await PermissionModel.findOne({ code: 'ADMIN_PANEL_FULL_ACCESS' });
      if (superAdminRole && adminPanelFullAccessPermission) {
        await PermissionModel.findByIdAndUpdate(adminPanelFullAccessPermission._id, {
          $addToSet: { allowedRoleIds: superAdminRole._id }
        });
        logger.info('[DatabaseSeeder] Successfully linked admin panel full access permission to super admin role');
      }
      if (identityManagerRole && adminPanelFullAccessPermission) {
        await PermissionModel.findByIdAndUpdate(adminPanelFullAccessPermission._id, {
          $addToSet: { allowedRoleIds: identityManagerRole._id }
        });
        logger.info('[DatabaseSeeder] Successfully linked admin panel full access permission to identity manager role');
      }
      ...
      logger.error('[DatabaseSeeder] Failed to link roles and permissions:', error);
      throw error;
    }
  }
  ...
  private async verifyLinking(): Promise<void> {
    try {
      logger.info('[DatabaseSeeder] Verifying role and permission linking...');
      
      // Check super admin user
      const superAdminUser = await UserModel.findOne({ email: 'admin@domain.com' });
      if (superAdminUser) {
        logger.info(`[DatabaseSeeder] ✅ Super admin user found: ${superAdminUser.email}`);
        logger.info(`[DatabaseSeeder] ✅ User roleIds: ${superAdminUser.roleIds?.join(', ') || 'None'}`);
        logger.info(`[DatabaseSeeder] ✅ User isAdmin: ${superAdminUser.isAdmin}`);
        
        // Check if user has SUPER_ADMIN role
        if (superAdminUser.roleIds && superAdminUser.roleIds.length > 0) {
          const superAdminRole = await RoleModel.findById(superAdminUser.roleIds[0]);
          if (superAdminRole && superAdminRole.code === 'SUPER_ADMIN') {
            logger.info(`[DatabaseSeeder] ✅ User has SUPER_ADMIN role: ${superAdminRole.name}`);
          } else {
            logger.warn(`[DatabaseSeeder] ⚠️ User role is not SUPER_ADMIN: ${superAdminRole?.code || 'Unknown'}`);
          }
        } else {
          logger.warn('[DatabaseSeeder] ⚠️ User has no roleIds assigned');
        }
      } else {
      ...
      if (superAdminPermission) {
        logger.info(`[DatabaseSeeder] ✅ Super admin permission found: ${superAdminPermission.name}`);
        logger.info(`[DatabaseSeeder] ✅ Permission allowedRoleIds: ${superAdminPermission.allowedRoleIds?.join(', ') || 'None'}`);
        logger.info(`[DatabaseSeeder] ✅ Permission allowedUserIds: ${superAdminPermission.allowedUserIds?.join(', ') || 'None'}`);
      } else {
        logger.warn('[DatabaseSeeder] ⚠️ Super admin permission not found');
      }
      
      logger.info('[DatabaseSeeder] Role and permission linking verification completed');
    } catch (error) {
  ...
  private async seedEmailconfigs(emailconfigs: any[]): Promise<void> {
    ...
    logger.info(`[DatabaseSeeder] Seeding ${emailconfigs.length} email configs...`);
    
    for (const config of emailconfigs) {
      try {
        const exists = await EmailConfigModel.findOne({ code: config.code });
        if (!exists) {
          await EmailConfigModel.create(config);
          logger.info(`[DatabaseSeeder] Created email config: ${config.code}`);
        } else {
          logger.info(`[DatabaseSeeder] Email config ${config.code} already exists, skipping`);
        }
      } catch (error) {
        logger.error(`[DatabaseSeeder] Failed to create email config ${config.code}:`, error);
