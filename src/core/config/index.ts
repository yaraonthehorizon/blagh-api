...
export class ConfigManager {
  ...
  private loadEnvironmentVariables(): void {
    ...
    const envFiles = [    // Base template (lowest priority)
    ...
    for (const envFile of envFiles) {
      ...
      if (fs.existsSync(envPath)) {
        console.log(`[ConfigManager] Loading environment file: ${envFile}`);
        const result = dotenv.config({ path: envPath });
        
        if (result.error) {
          console.warn(`[ConfigManager] Warning: Failed to load ${envFile}:`, result.error.message);
        } else {
          console.log(`[ConfigManager] Successfully loaded ${envFile}`);
        }
      } else {
        console.log(`[ConfigManager] Environment file not found: ${envFile}`);
      }
    }

    // Log the final environment being used
    console.log(`[ConfigManager] Environment: ${environment}`);
    console.log(`[ConfigManager] Node environment: ${process.env.NODE_ENV || 'not set'}`);
   ...
   */
  getEnvironmentInfo(): { environment: string; loadedFiles: string[]; availableFiles: string[] } {
    const environment = process.env.NODE_ENV || 'development';
    const cwd = process.cwd();
   ...
   */
  validateEnvironment(): { isValid: boolean; issues: string[]; recommendations: string[] } {
    const issues: string[] = [];
    ...
    if (envInfo.availableFiles.length === 0) {
      ...
      recommendations.push('Create a .env file for your local configuration');
    }

    // Check if environment-specific file exists
    const envSpecificFile = `.env.${envInfo.environment}`;
    if (!envInfo.availableFiles.includes(envSpecificFile)) {
      recommendations.push(`Consider creating ${envSpecificFile} for ${envInfo.environment}-specific settings`);
    }
    ...
    for (const requiredVar of requiredVars) {
      if (!process.env[requiredVar]) {
        issues.push(`Missing required environment variable: ${requiredVar}`);
      }
    }
    ...
    return {
      ...
      auth: {
        ...
        passwordPolicy: {
          ...
          requireSpecialChars: process.env.AUTH_PASSWORD_REQUIRE_SPECIAL_CHARS !== 'false'
        },
        providers: {
          internal: {
            enabled: process.env.INTERNAL_AUTH_ENABLED !== 'false',
            allowRegistration: process.env.INTERNAL_AUTH_ALLOW_REGISTRATION !== 'false',
            requireEmailVerification: process.env.INTERNAL_AUTH_REQUIRE_EMAIL_VERIFICATION === 'true',
            allowPasswordReset: process.env.INTERNAL_AUTH_ALLOW_PASSWORD_RESET !== 'false',
            bcryptRounds: parseInt(process.env.INTERNAL_AUTH_BCRYPT_ROUNDS || '12'),
            database: {
              userTable: process.env.INTERNAL_AUTH_USER_TABLE || 'users',
              tokenTable: process.env.INTERNAL_AUTH_TOKEN_TABLE || 'auth_tokens',
              sessionTable: process.env.INTERNAL_AUTH_SESSION_TABLE || 'user_sessions'
            }
          },
          firebase: {
            ...
            adminSDK: {
              type: process.env.FIREBASE_ADMIN_SDK_TYPE || 'service_account',
          ...
          oauth: {
            ...
            providers: {
              ...
              twitter: {
                ...
                scope: process.env.TWITTER_SCOPE ? process.env.TWITTER_SCOPE.split(',') : ['tweet.read', 'users.read']
              },
              linkedin: {
                ...
                scope: process.env.LINKEDIN_SCOPE ? process.env.LINKEDIN_SCOPE.split(',') : ['r_liteprofile', 'r_emailaddress']
              },
              microsoft: {
                ...
                scope: process.env.MICROSOFT_SCOPE ? process.env.MICROSOFT_SCOPE.split(',') : ['openid', 'profile', 'email']
              },
              apple: {
                ...
                scope: process.env.APPLE_SCOPE ? process.env.APPLE_SCOPE.split(',') : ['name', 'email']
              },
              discord: {
                ...
                scope: process.env.DISCORD_SCOPE ? process.env.DISCORD_SCOPE.split(',') : ['identify', 'email']
              },
              spotify: {
                ...
                scope: process.env.SPOTIFY_SCOPE ? process.env.SPOTIFY_SCOPE.split(',') : ['user-read-email']
              },
              twitch: {
                enabled: process.env.TWITCH_OAUTH_ENABLED === 'true',
                clientId: process.env.TWITCH_CLIENT_ID || '',
                clientSecret: process.env.TWITCH_CLIENT_SECRET || '',
                callbackUrl: process.env.TWITCH_CALLBACK_URL || '/auth/twitch/callback',
                scope: process.env.TWITCH_SCOPE ? process.env.TWITCH_SCOPE.split(',') : ['user:read:email']
              }
            }
          },
          sms: {
            enabled: process.env.SMS_ENABLED === 'true',
            provider: (process.env.SMS_PROVIDER || 'twilio') as 'twilio' | 'aws-sns' | 'vonage' | 'messagebird' | 'custom',
            codeLength: parseInt(process.env.SMS_CODE_LENGTH || '6'),
            codeExpiration: parseInt(process.env.SMS_CODE_EXPIRATION || '300'),
          ...
          email: {
            ...
            provider: 'nodemailer' as 'nodemailer' | 'sendgrid' | 'aws-ses' | 'mailgun' | 'postmark' | 'custom',
            codeLength: parseInt(process.env.EMAIL_CODE_LENGTH || '6'),
            codeExpiration: parseInt(process.env.EMAIL_CODE_EXPIRATION || '300'),
            ...
            templates: {
              ...
              loginCode: process.env.EMAIL_LOGIN_CODE_TEMPLATE || 'login-code'
            }
          }
        }
      },
      ...
      odoo: {
        ...
        models: {
          users: {
            enabled: process.env.ODOO_MODEL_USERS_ENABLED !== 'false',
            model: 'res.users',
            fields: ['id', 'name', 'email', 'login', 'active', 'company_id', 'partner_id'],
            mapping: {},
          ...
          customers: {
            enabled: process.env.ODOO_MODEL_CUSTOMERS_ENABLED !== 'false',
            model: 'res.partner',
          ...
          leads: {
            enabled: process.env.ODOO_MODEL_LEADS_ENABLED !== 'false',
            model: 'crm.lead',
            fields: ['id', 'name', 'partner_name', 'contact_name', 'email_from', 'phone', 'type', 'stage_id'],
            mapping: {},
            syncDirection: 'bidirectional',
            createOnSync: true,
            updateOnSync: true
          ...
          opportunities: {
            enabled: process.env.ODOO_MODEL_OPPORTUNITIES_ENABLED !== 'false',
            model: 'crm.lead',
            fields: ['id', 'name', 'partner_id', 'contact_name', 'email_from', 'phone', 'type', 'stage_id', 'probability'],
            mapping: {},
            syncDirection: 'bidirectional',
            createOnSync: true,
            updateOnSync: true
          ...
          contacts: {
            enabled: process.env.ODOO_MODEL_CONTACTS_ENABLED !== 'false',
            model: 'res.partner',
          ...
          companies: {
            enabled: process.env.ODOO_MODEL_COMPANIES_ENABLED !== 'false',
            model: 'res.company',
          ...
          products: {
            enabled: process.env.ODOO_MODEL_PRODUCTS_ENABLED !== 'false',
            model: 'product.template',
            fields: ['id', 'name', 'description', 'list_price', 'standard_price', 'type'],
            mapping: {},
            syncDirection: 'inbound',
            createOnSync: false,
            updateOnSync: true
          ...
          orders: {
            enabled: process.env.ODOO_MODEL_ORDERS_ENABLED !== 'false',
            model: 'sale.order',
          ...
          invoices: {
            enabled: process.env.ODOO_MODEL_INVOICES_ENABLED !== 'false',
            model: 'account.move',
        ...
        sync: {
          ...
          batchSize: parseInt(process.env.ODOO_SYNC_BATCH_SIZE || '100'),
          retryAttempts: parseInt(process.env.ODOO_SYNC_RETRY_ATTEMPTS || '3'),
          retryDelay: parseInt(process.env.ODOO_SYNC_RETRY_DELAY || '1000')
        },
        webhooks: {
          enabled: process.env.ODOO_WEBHOOKS_ENABLED === 'true',
          secret: process.env.ODOO_WEBHOOKS_SECRET || '',
          events: process.env.ODOO_WEBHOOKS_EVENTS ? process.env.ODOO_WEBHOOKS_EVENTS.split(',') : []
        }
      }
    };
  }
    ...
    return {
      ...
      auth: {
        ...
        providers: {
          ...
          sms: {
            enabled: false,
            provider: 'twilio' as 'twilio' | 'aws-sns' | 'vonage' | 'messagebird' | 'custom',
            codeLength: 6,
            codeExpiration: 300,
            maxAttempts: 3
          },
